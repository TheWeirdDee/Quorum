...
describe("pollOnce", () => {
  ...
  it("processes every newly-admitted event for every registered repo", async () => {
    const repoA = upsertRepo(db, { githubUrl: "https://github.com/acme/a", riskPolicy: "balanced" });
    const repoB = upsertRepo(db, { githubUrl: "https://github.com/acme/b", riskPolicy: "enterprise" });
  ...
  it("logs and continues sweeping other repos when event detection fails for one", async () => {
    upsertRepo(db, { githubUrl: "https://github.com/acme/broken", riskPolicy: "balanced" });
    upsertRepo(db, { githubUrl: "https://github.com/acme/ok", riskPolicy: "balanced" });
  ...
  it("logs and continues when processEvent throws for one event", async () => {
    upsertRepo(db, { githubUrl: "https://github.com/acme/c", riskPolicy: "balanced" });
  ...
  it("does nothing for a repo with no newly-admitted events", async () => {
    upsertRepo(db, { githubUrl: "https://github.com/acme/quiet", riskPolicy: "balanced" });
...
describe("startPollLoop", () => {
  ...
  it("runs pollOnce on each interval and stop() halts further cycles", async () => {
    vi.useFakeTimers();
    try {
      upsertRepo(db, { githubUrl: "https://github.com/acme/timer", riskPolicy: "balanced" });
